...
app.controller('playlistController', function($scope, $route, $location) {
    ...
    if (localStorage.getItem("PlayListArray") === null) {} else {
...
app.controller('MainController', function($scope, $http, $location) {
    ...
    function fetch() {
        ...
        $scope.songs = [];
        $http.get("API_URL=" + $scope.search + "&perpage=500")
            .success(function(response) {
                ...
                for (var i = 0; i < response.tracks.length; i++) {
                    ...
                    var resultUrl = "API_URL" + labelLow + "&alb=" + response.tracks[i].album_num + "&trk=" + response.tracks[i].track_num + "";
                    var imgUrl = "API_URL" + labelLow + "/" + response.tracks[i].album_num + "";
